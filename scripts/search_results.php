...
function getCityState($nid) {
  $query = db_query("SELECT li.lid, l.street, l.city, l.province, l.postal_code FROM {location_instance} li, {location} l WHERE li.lid = l.lid AND li.genid LIKE 'cck:field_location%' AND li.nid = :nid LIMIT 1", array(':nid' => $nid));
  //while ($row = db_fetch_object($query)) {
  foreach($query as $row) {
...
function displayMap($results, $loc, $zoom, $smallMap) {
  ...
  if (count($results) > 0) {
    ...
    foreach($results as $nid) {
      ...
      // Get rates
      $rates = "";
//      $query2 = db_query("SELECT DISTINCT td.name FROM {term_node} tn, {node} n, {term_data} td WHERE tn.nid = n.nid AND tn.tid = td.tid AND n.nid = :nid AND td.vid = 11 ORDER BY td.weight ASC", array(':nid' => $nid));
      $query2 = db_query("SELECT DISTINCT td.name FROM {field_data_taxonomy_vocabulary_11} tn, {node} n, {taxonomy_term_data} td WHERE tn.entity_id = n.nid AND tn.taxonomy_vocabulary_11_tid = td.tid AND n.nid = :nid AND td.vid = 11 ORDER BY td.weight ASC", array(':nid' => $nid));
...
function getResultAlias($nid) {
  ...
  return url($target);
/*
  $query = db_query("SELECT dst FROM {url_alias} WHERE src = :target", array(':target' => $target));
  //while ($row = db_fetch_object($query)) {
  foreach($query as $row) {
    $result = $row->dst;
  }
...
function recordSearch($loc, $terms) {
  ...
  $loc = str_replace('"', "", $loc);
  db_query("INSERT INTO {gca_searches} (ip, created, data, filters) VALUES (:ip, :timenow, :loc, :terms)", array(':ip' => $ip, ':timenow' => $timenow, ':loc' => $loc, ':terms' => $terms));
}
