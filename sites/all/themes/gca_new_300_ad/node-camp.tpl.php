<?php
  ...
  $tier = $nodeInfo->field_park_tier[0]["value"];
  //echo "<pre>";
  //print_r($nodeInfo);
  //print_r($user);
  //echo "</pre>";
  ...
  $stateTemp = explode("/", $node->path);
  $state = ucwords(str_replace("-", " ", $stateTemp[1]));
  ...
  if ($nodeInfo->field_park_state_assn_optin[0]["value"] == "on" && $nodeInfo->field_camp_state_assnid[0]["value"]) {
	...
	//echo $timeNow . " | ";
    $query = db_query("SELECT DISTINCT n.nid FROM {node} n WHERE n.type = 'deal' AND n.uid = %d", $assnUID);
	while ($row = db_fetch_array($query)) {
	  //echo $row["nid"] . " ";
	  $couponVID = getNewestVID($row["nid"]);
	  //echo "v" . $couponVID . " ";
	  $query2 = db_query("SELECT field_deal_start_value, field_deal_end_value FROM {content_type_deal} WHERE nid = %d AND vid = %d", $row["nid"], $couponVID);
  ...
  $query = db_query("SELECT nid FROM {node} WHERE type = 'deal' AND uid = %d", $node->uid);
  while ($row = db_fetch_object($query)) {
    $coupon[] = $row->nid;
  }
  $query = db_query("SELECT n.nid FROM {node} n, {content_type_events} cte WHERE n.nid = cte.nid AND n.type = 'events' AND n.uid = %d", $node->uid);
  while ($row = db_fetch_object($query)) {
  ...
  <div class="content">
    ...
    <div id="park-tabs">
      <ul>
		...
		<?php 
	    if ($nodeInfo->field_camp_guestreview_id[0]["value"] && $nodeInfo->field_park_guest_reviews_optin[0]["value"] == "yes") { ?>
        ...
        <?php } ?>
        <?php if (count($node->field_camp_video_url) && ($tier > 2 || $nodeInfo->field_camp_swamp_city[0]["value"] == "on")) { ?>
        <li <?php if ($activeTab == "videos") { echo 'class="selected" '; } ?> rel="#park-videos" tab="videos">Videos</li>
        <?php } ?>
        <?php if ($node->field_camp_slideshow[0][filepath]) { ?>
        <li <?php if ($activeTab == "photos") { echo 'class="selected" '; } ?> rel="#park-photos" tab="photos">Photos</li>
        <?php } ?>
        <?php if ($node->field_camp_email[0]['email']) { ?>
        ...
        <div id="park-info-left">
          <div id="park-info-name" class="ui-corners-all">
            ...
            <div id="park-info-address">
              <?php echo $node->field_location[0][street] ."<br />\n" . 
              $node->field_location[0][city] . ", " . $node->field_location[0][province] . " " . $node->field_location[0][postal_code] . "<br />\n"; ?>
              
            </div>
            <?php if ($node->field_camp_phone[0]['number']) { ?>
            <div id="park-phone">
              <?php if (strlen($node->field_camp_phone[0]['number']) == 10) {
                $phonenumber = "(" . substr($node->field_camp_phone[0]['number'], 0, 3) . ") " . substr($node->field_camp_phone[0]['number'], 3, 3) . "-" . substr($node->field_camp_phone[0]['number'], 6, 4);
              } else {
                $phonenumber = $node->field_camp_phone[0]['number'];
              } ?>
            ...
            <?php } ?>
            <?php if ($node->field_camp_website[0]['url']): ?>
            <div id="park-website_url">
            	<a href="<?php print $node->field_camp_website[0]['url'] ?>" target="new">Visit Our Website</a>
            </div>
            <?php endif; ?>
			<?php if ($node->field_camp_reservation_website[0]["url"]) {
			  echo "<div id='park-reservation'><a href='" . $node->field_camp_reservation_website[0]["url"] . "'>Make Reservation</a></div>";
			} ?>
			<?php if ($socialInfo["facebook"] || $socialInfo["twitter"]) { ?>
			<div id="park-social-media">
			  <?php if ($socialInfo["facebook"]) { ?>
			  <a href="<?php echo $socialInfo["facebook"]; ?>" target="new"><img src="/sites/all/themes/gca_new_interior/images/icon_facebook_sm.gif" alt="Visit us on Facebook" style="margin:5px 5px 0 0;float:left;" /></a>
			  <?php } ?>
			  <?php if ($socialInfo["twitter"]) { ?>
			  <a href="<?php echo $socialInfo["twitter"]; ?>" target="new"><img src="/sites/all/themes/gca_new_interior/images/icon_twitter_sm.gif" alt="Visit us on Twitter" style="margin-top:5px;float:left;" /></a>
			  <?php } ?>
			  <br clear="all" />
			</div>
			<?php } 
			if ($node->field_camp_open_year_round[0]["value"] == "on") {
			  echo "Open Year Round<br />";
			} else {
			  if ($node->field_park_date_open[0]["value"] && $node->field_park_date_open_day[0]["value"]) {
			    echo "<b>Opens:</b> " . $node->field_park_date_open[0]["value"] . " " . $node->field_park_date_open_day[0]["value"] . "<br />";
			  } 
			  if ($node->field_park_date_closed_month[0]["value"] && $node->field_park_date_closed_day[0]["value"]) { 
			    echo "<b>Closes:</b> " . $node->field_park_date_closed_month[0]["value"] . " " . $node->field_park_date_closed_day[0]["value"] . "<br />";
		      } 
			} ?>
            <div id="park-info-google-link">
              <?php $googleString = str_replace(" ", "+", $node->field_location[0][street] . ", " . $node->field_location[0][city] . " " . $node->field_location[0][province]); ?>
          ...
          <div id="park-info-map" class="ui-corners-all">
            <a href="http://www.google.com/maps?q=<?php echo $googleString; ?>&t=h&z=16" target="_blank"><img src="http://maps.googleapis.com/maps/api/staticmap?center=<?php echo $node->field_location[0][latitude] ?>,<?php echo $node->field_location[0][longitude] ?>&zoom=11&size=200x200&
markers=color:blue%7Clabel:%7C<?php echo $node->field_location[0][latitude] ?>,<?php echo $node->field_location[0][longitude] ?>&sensor=false" /></a>
          </div> <!-- /#park-info-map -->
        </div> <!-- /#park-info-left -->
        <div id="park-info-right">
          <div id="park-info-body">
		    <?php if ($nodeInfo->field_planit_green[0]["value"] == "yes") { ?>
			...
			<?php } 
            echo "<div id='park-description'>" . $node->field_park_description[0]["safe"] . "</div>";
			if ($node->field_camp_directions[0]["safe"]) {
			  echo "<div id='park-directions'><b>Directions:</b> " . $node->field_camp_directions[0]["safe"] . "</div>";
			}
			if ($node->field_camp_rules[0]["safe"]) {
			  echo "<div id='park-rules'><b>Park Rules:</b> " . $node->field_camp_rules[0]["safe"] . "</div>";
			}
			if ($node->field_camp_local_interest[0]["safe"]) {
			  echo "<div id='park-local-interest'><b>Local Interest:</b> " . $node->field_camp_local_interest[0]["safe"] . "</div>";
			}
			?>
          </div> <!-- /#park-info-body -->
		  ...
		  <div id="park-info-state">
            ...
            <?php			
              $id = $node->field_location[0]["province"];
              $state = $node->field_location[0]["province_name"];
          ...
          <br clear="all" />
		  <?php if ($nodeInfo->field_planit_green[1]["value"] == "no") { ?>
	  ...
	  <?php 
	  if ($nodeInfo->field_park_guest_reviews_optin[0]["value"] == "yes") { ?>
      <div id="park-reviews" class="park-tab-content<?php if ($activeTab != 'reviews') { echo ' hide'; } ?>">
        <b><a href="#reviewPark" class="ui-corners-all" style="background:#7DB02A;color:#FFF;text-align:center;padding:5px;width:250px;">Review this park</a></b><br /><br />        <iframe id="MostRecentReviewsSinglePropertyIFrame" frameborder="0" scrolling="no" src="http://travel.guestrated.com/Widget/Pages/MostRecentReviewsSingleProperty.aspx?custtypeid=8&amp;portalid=518&amp;customerid=<?php echo $node->field_camp_guestreview_id[0][value]; ?>" width="800px" height="1600px" allowtransparency="true"></iframe>
        <a name="reviewPark"></a>
        <div class="review-park" style="background:#FFF;width:600px;">
          <IFRAME id="ctl00_Main_myFrame" width="600" height="1400" scrolling="no" frameborder="0" src="http://survey.guestrated.com/TakeSurvey.aspx?SurveyID=m2KH552&amp;customerID=<?php echo $node->field_camp_guestreview_id[0][value]; ?>&amp;portalID=61"></IFRAME>
		</div> <!-- /.review-park -->
  ...
  <div class="field field-type-filefield field-field-image couponImage">
    <div class="field-items">
      <div class="field-item"><img src="/<?php echo $couponInfo->field_deal_image[0]['filepath'] ?>" alt="" class="imagecache imagecache-coupon_logo imagecache-default imagecache-coupon_logo_default" width="153" height="193" /></div>
    </div>
  </div>
  <div class="couponData">
    ...
    <div class="couponDtDescrWrap">
      <div class="couponDescr" id="couponDescr">
        ...
        <?php echo $couponInfo->field_deal_description[0]['value']; ?>
	  </div>
      <div class="field field-type-date field-field-startdate couponDate">
        <div class="field-item">
		Expires<br />
		<?php echo date("n/j/Y", strtotime($couponInfo->field_deal_end[0]['value'])); ?>
		</div>
      </div>
    </div>
  </div>
      ...
      <div id="park-videos" class="park-tab-content<?php if ($activeTab != 'videos') { echo ' hide'; } ?>">
        ...
        if ($videos[0]["url"] != "") {
		  ...
		  $videos[0]["id"] = str_replace($toReplace, "", $videos[0]["url"]);
		  //echo $videos[0]["id"] . "<br />";
		  if (strpos($videos[0]["id"], "&") > 0) {
		    $videos[0]["id"] = substr($videos[0]["id"], 0, strpos($videos[0]["id"], "&"));
          }
        }
        if ($videos[0]["url"] != "") {
        ?>
        <div id="park-video-wrapper" class="ui-corners-all">
          <div class='park-video-player'>
            <div class='park-video-player-title'><?php echo $videos[0]["title"] ?></div>
          ...
          <?php
          if (count($videos) > 0 && ($tier > 2 || $nodeInfo->field_camp_swamp_city[0]["value"])) {
            $end = round(count($videos) / 2);
            echo "<div id='park-video-playlist'>";
            echo "<div class='left park-playlist-col col1'>";
            echo "<div class='park-video-playlist-item' rel='" . $videos[0]["id"] . "' title='" . addslashes($videos[0]["title"]) . "'><img src='http://i3.ytimg.com/vi/" . $videos[0]["id"] . "/default.jpg' width='120' height='90' align='left' />" . $videos[0]["title"] . "</div>";
            echo "</div>";
      ...
      <?php 
        if ($node->field_camp_slideshow[0][filepath]) {
		  switch ($tier) {
			  case 1:
			    $photoLimit = 3;
				break;
          ...
          for ($i = 0; $i < $photoLimit; $i++) {
		    if ($node->field_camp_slideshow[$i][filepath]) {
              $imageSize = getimagesize("/var/www/vhosts/gocampingamerica.com/httpdocs/" . $node->field_camp_slideshow[$i][filepath]);
              $width = $imageSize[0];
              $height = $imageSize[1];
              if ($width > $height) {
                $width = 150;
              ...
              echo "<a href='#photo-slideshow'><img src='/" . $node->field_camp_slideshow[$i][filepath] . "' width='" . $width . "' height='" . $height . "' /></a>";
              echo "</div>";
			  if ($column == 4) {
			    echo "<br clear='all' />";
				$column = 1;
          ...
          $x = 0;
		  $picCount = 0;
		  foreach ($node->field_camp_slideshow as $pic) {
		    if ($pic["filepath"]) {
			  $picCount++;
			}
		  }
      ...
      <div id="park-contact" class="park-tab-content<?php if ($activeTab != 'contact') { echo ' hide'; } ?>">
        ...
        <?php
          require('/var/www/vhosts/gocampingamerica.com/httpdocs/scripts/recaptchalib.php');
...
<?php
function getFBT($nid) {
  $query = db_query("SELECT field_park_facebook_url, field_park_twitter_url FROM {content_type_camp} WHERE nid = %d", $nid);
  while ($row = db_fetch_array($query)) {
...
function getResources($state) {
  $query = db_query("SELECT sr.delta, sr.field_state_resources_url, sr.field_state_resources_title FROM {node} n, {content_field_state_resources} sr WHERE n.nid = sr.nid AND n.title = '%s' ORDER BY sr.delta ASC", $state);
  $x = 0;
  while ($row = db_fetch_object($query)) {
