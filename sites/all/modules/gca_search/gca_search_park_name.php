...
function getInfo($nids) {
  $x = 0;
  foreach ($nids as $nid) {
    $query = db_query("SELECT n.title, l.city, l.province FROM {node} n, {content_type_camp} ctc, {location} l, {location_instance} li WHERE n.nid = ctc.nid AND n.nid = li.nid AND li.lid = l.lid AND li.genid LIKE '%%field_location%%' AND n.nid = :nid", array("nid" => $nid));
    while ($row = $query->fetchObject()) {
      $results[$x][nid] = $nid;
